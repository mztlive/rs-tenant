...
- `source.rs` defines the async `AuthorizationSource` trait.
- `ids.rs`, `permission.rs`, `scope.rs`, `role.rs`, `request.rs`, `decision.rs`, and `error.rs` define domain types and validation.
- Optional adapters are feature-gated (`memory_source.rs`, `memory_cache.rs`, `axum.rs`).
...
- `cargo test --offline --features memory-store,memory-cache` runs cache/source feature tests.
